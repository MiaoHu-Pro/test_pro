...
if __name__ == "__main__":
    ...
    entity2id_path = "./hadoop_data_with_time/entity2id.txt"
    relation2id_path = "./hadoop_data_with_time/relation2id.txt"
    entity2id_dic, relation2id_dic = get_entity_relation_2id(entity2id_path, relation2id_path)

    _train = read_files('./hadoop_data_with_time/test.txt')

    head = _train[:, 0]
    rel = _train[:, 1]
    tail = _train[:, 2]

    entity = list(set(list(set(head)) + list(set(tail))))
    ...
    training_entity2id = []
    training_relation2id = []

    for i in rel:
        _rel = []
        if i in relation2id_dic.keys():
            _rel.append(i)
            _rel.append(relation2id_dic.get(i))
            training_relation2id.append(_rel)

    print(len(training_relation2id))

    write_data_2_id("./hadoop_data_with_time/test_relation2id.txt", training_relation2id)

    for i in entity:
        _ent = []
        if i in entity2id_dic.keys():
            _ent.append(i)
            _ent.append(entity2id_dic.get(i))
            training_entity2id.append(_ent)

    print(len(training_entity2id))
    # print(training_entity2id)

    write_data_2_id("./hadoop_data_with_time/test_entity2id.txt", training_entity2id)

    # obtain_trple2id(entity2id_dic,relation2id_dic,_train,"unseen_both_entity_test2id")
