...
if __name__ == "__main__":
    total_triples = read_files("hadoop_data_process/valid.txt")

    relation_list = total_triples[:, 1].tolist()

    static_frequency("hadoop_data_process/valid_triples_relation_frequency.txt", relation_list)

    entity_list = total_triples[:, 0].tolist() + total_triples[:, 2].tolist()

    static_frequency("hadoop_data_process/valid_triples_entity_frequency.txt", entity_list)
